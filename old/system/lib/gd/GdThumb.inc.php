...
class GdThumb extends ThumbBase
{
	...
	public function adaptiveResize2 ($width, $height)
	{
		// make sure our arguments are valid
		if (!is_numeric($width) || $width  == 0)
		{
	...
	public function show () 
	{
		...
		switch ($this->format) 
		{
			...
			case 'JPG':
				...
				imagejpeg($im, null, $this->options['jpegQuality']);
				break;
			case 'PNG':
	...
	public function showWatermask ($result_pic_water) 
	{
		if (headers_sent())
		{
			throw new RuntimeException('Cannot show image, headers have already been sent');
		}
		// echo $result_pic_water;exit();
		// $url = $result_pic_water;
		// $ch = curl_init();
		...
		$stamp = imagecreatefrompng($result_pic_water);
		// var_dump($stamp);exit();
